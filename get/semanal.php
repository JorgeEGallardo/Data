...
<?php
$date = "2019-09-01";//$_POST["date"]; //Fecha recibida desde el input
$Fechain = date("Y-m-d",strtotime('monday this week', strtotime($date))); //Calculamos el primer dia de la semana
$FechaFin = date("Y-m-d",strtotime('sunday this week', strtotime($date)));//Y el ultimo
$bd = 0;//$_POST["bd"]; //Base de datos a analizar
include('../config/servicio.php'); // Librerias y configuracion necesarias para jalar los datos
$fetchExival = array(); //Aqui vamos a meter los resultados de la base de datos
$fetch
 = array(); 
$fetchExival = array(); 
$fetchExival = array(); 
$fetchExival = array(); 
$fetchExival = array(); 
for($i = 0; $i<7 ;$i++){ //For para repetir el proceso y cubrir los 7 dias de la semana
$date = date("Y-m-d", strtotime("+".$i."days", strtotime($Fechain))); //Recorremos los dias uno por cada ciclo
$conn=ibase_connect($servicedini.":".$rutadini."SEMANAL.FDB",$usuariodini, $basedecode);	//Conexion a la base
$Query = "SELECT VALOR FROM EXIVAL WHERE BD = '$empresadini[$bd]' AND FECHA = '$date';";  //Query traemos el valor de la base de datos en donde la empresa y la fecha sean iguales a los parametros.
$Exec = ibase_query($conn, $Query); //Ejecucion del query
$rowV = 0;
while ($row = ibase_fetch_object($Exec)) {
    $rowV = $row->VALOR;
} 
array_push($fetchExival,$rowV); //Llenamos el array con los resultados

}
$arrayExival = implode(",",$fetchExival); //Convertimos el array en texto para poder manejarlo en el Json
$jsons = array( //String que contiene la informacion de la grafica
    "{label: 'Inventarios', borderColor: 'green',  backgroundColor: 'orange',  fill: false,  data:[$arrayExival],yAxisID: 'y-axis-1'}",
